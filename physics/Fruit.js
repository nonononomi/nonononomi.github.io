...
class Fruit {
  ...
  merge(b) {
    ...
    //window.score += points[this.type];

    if (window.onMerge) {
    window.onMerge(this.type);
}
...
let data = {
  ...
  berry: {
    color: 'crimson',
    size:  20,
    shape: scaleShape(ShapeBerry, .4),
    next:  'grape',
  },
  grape: {
    color: 'purple',
    size:  30,
    shape: scaleShape(ShapeGrape, .2),
    next:  'banana',
  },
  banana: {
    color: '#cccc00',
    shape: scaleShape(ShapeBanana, .7),
    next: 'orange',
  },
  orange: {
    color: 'orange',
  ...
  apple: {
    color: '#991100',
    shape: scaleShape(ShapeApple, 1.6),
    next: 'melon',
  },
  melon: {
    color: '#88ff88',
    shape: scaleShape(ShapeMelon, 1),
    next: 'suika',
  },
